...
func selectorStatsMetadataFromSpec(spec *v1alpha1.TracingPolicySpec) []selectorStatsMetadata {
	...
	for _, kprobe := range spec.KProbes {
		ret = appendSelectorStatsMetadata(ret, "kprobe:"+kprobe.Call, kprobe.Selectors)
	}
	for _, fentry := range spec.Fentries {
		ret = appendSelectorStatsMetadata(ret, "fentry:"+fentry.Call, fentry.Selectors)
	}
	for _, uprobe := range spec.UProbes {
		ret = appendSelectorStatsMetadata(ret, uprobeStatsHook(uprobe), uprobe.Selectors)
	}
	for _, tp := range spec.Tracepoints {
		ret = appendSelectorStatsMetadata(ret, "tracepoint:"+tp.Subsystem+"/"+tp.Event, tp.Selectors)
	}
	for _, lsm := range spec.LsmHooks {
		ret = appendSelectorStatsMetadata(ret, "lsm:"+lsm.Hook, lsm.Selectors)
	}
	for _, usdt := range spec.Usdts {
		ret = appendSelectorStatsMetadata(ret, "usdt:"+usdt.Path+":"+usdt.Provider+"/"+usdt.Name, usdt.Selectors)
	}
	return ret
}

func appendSelectorStatsMetadata(ret []selectorStatsMetadata, hook string, selectors []v1alpha1.KProbeSelector) []selectorStatsMetadata {
